...
from version_utils import read_version_marker, DEFAULT_PLUGIN_VERSION
...
def _format_request_error(error: Exception) -> str:
    ...
    if "wrong version number" in lowered and "ssl" in lowered:
        return (
            "SSL/TLS error: server returned plain HTTP to an HTTPS request. "
            "Check API URL (http vs https) or any proxy/AV interception."
        )
...
class HTTPClient:
    ...
    def _get_cached_headers(self) -> Dict[str, str]:
        if self._cached_headers is None:
            ...
            try:
                plugin_version = read_version_marker()
            except Exception:
    ...
    def get(self, url: str, params: Optional[Dict[str, Any]] = None, auth_token: Optional[str] = None, extra_headers: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        try:
            client = self._ensure_client()
            headers = self._get_cached_headers().copy()
            ...
            if HTTPX_AVAILABLE and HTTPStatusError is not None and isinstance(e, HTTPStatusError):
                ...
                error_msg = _format_request_error(e)
    ...
    def get_binary(self, url: str, params: Optional[Dict[str, Any]] = None, auth_token: Optional[str] = None, extra_headers: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        try:
            client = self._ensure_client()
            headers = self._get_cached_headers().copy()
    ...
    def post(self, url: str, data: Optional[Dict[str, Any]] = None, auth_token: Optional[str] = None, extra_headers: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        try:
            client = self._ensure_client()
            headers = self._get_cached_headers().copy()
    ...
    def clear_cached_headers(self) -> None:
        self._cached_headers = None
...
def clear_global_client_headers() -> None:
    global _global_client
    if _global_client is not None:
        _global_client.clear_cached_headers()
